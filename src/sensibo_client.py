...
class SensiboClientAPI(object):
    ...
    def pod_measurement(self, podUid):
        result = self._get(f"/pods/{podUid}/measurements")
        return result['result']

    def pod_ac_state(self, podUid):
        result = self._get(f"/pods/{podUid}/acStates", limit=1,
                           fields="status,reason,acState")
        return result['result'][0]['acState']

    def pod_change_ac_state(self, podUid, currentAcState,
                            propertyToChange, newValue):
        self._patch(f"/pods/{podUid}/acStates/{propertyToChange}",
                json.dumps({'currentAcState': currentAcState,
                            'newValue': newValue}))
...
if __name__ == "__main__":
    ...
    print("-" * 10, f"AC State of {args.devicename}", "_" * 10)
    print(ac_state)
